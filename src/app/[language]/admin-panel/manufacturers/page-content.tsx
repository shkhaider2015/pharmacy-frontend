...
import { useDeleteManufacturerService } from "@/services/api/services/manufacturers";

const TableCellLoadingContainer = styled(TableCell)(() => ({
  padding: 0,
}));

function Actions({ entityItem }: { entityItem: Manufacturer }) {
  const [open, setOpen] = useState(false);
  const { confirmDialog } = useConfirmDialog();
  const fetchDeleteManufacturer = useDeleteManufacturerService();
  const queryClient = useQueryClient();
  const anchorRef = useRef<HTMLDivElement>(null);
  const { t } = useTranslation("admin-panel-manufacturers");
  ...
  const handleDelete = async () => {
    ...
    if (isConfirmed) {
      ...
      const previousData = queryClient.getQueryData<
        InfiniteData<{ nextPage: number; data: Manufacturer[] }>
      >(manufacturersQueryKeys.list().key);
      ...
      queryClient.setQueryData(manufacturersQueryKeys.list().key, newData);

      await fetchDeleteManufacturer({
        id: entityItem.id,
      });
    }
  };
  ...
  const mainButton = (
    <Button
      ...
      variant="contained"
      LinkComponent={Link}
      href={`/admin-panel/manufacturers/edit/${entityItem.id}`}
          ...
          <Grid container size="auto" wrap="nowrap" spacing={2}>
            <Grid size="auto">
              <Button
                variant="contained"
                LinkComponent={Link}
                href="/admin-panel/manufacturers/create"
                color="success"
...
export default withPageRequiredAuth(ManufacturersPageContent, {
