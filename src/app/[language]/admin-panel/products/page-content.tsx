...
import { useGetProductsListQuery, productsQueryKeys } from "./queries/queries";
...
import { useDeleteProductService } from "@/services/api/services/products";

const TableCellLoadingContainer = styled(TableCell)(() => ({
  padding: 0,
}));

function Actions({ entityItem }: { entityItem: Product }) {
  const [open, setOpen] = useState(false);
  const { confirmDialog } = useConfirmDialog();
  const fetchDeleteProduct = useDeleteProductService();
  const queryClient = useQueryClient();
  const anchorRef = useRef<HTMLDivElement>(null);
  const { t } = useTranslation("admin-panel-products");
  ...
  const handleDelete = async () => {
    ...
    if (isConfirmed) {
      ...
      const previousData = queryClient.getQueryData<
        InfiniteData<{ nextPage: number; data: Product[] }>
      >(productsQueryKeys.list().key);
      ...
      queryClient.setQueryData(productsQueryKeys.list().key, newData);
  ...
  const mainButton = (
    <Button
      ...
      variant="contained"
      LinkComponent={Link}
      href={`/admin-panel/products/edit/${entityItem.id}`}
...
export default withPageRequiredAuth(ProductsPageContent, {
