...
import { useDeleteSupplierService } from "@/services/api/services/suppliers";

const TableCellLoadingContainer = styled(TableCell)(() => ({
  padding: 0,
}));

function Actions({ entityItem }: { entityItem: Supplier }) {
  const [open, setOpen] = useState(false);
  const { confirmDialog } = useConfirmDialog();
  const fetchDeleteSupplier = useDeleteSupplierService();
  const queryClient = useQueryClient();
  const anchorRef = useRef<HTMLDivElement>(null);
  const { t } = useTranslation("admin-panel-suppliers");
  ...
  const handleDelete = async () => {
    ...
    if (isConfirmed) {
      ...
      const previousData = queryClient.getQueryData<
        InfiniteData<{ nextPage: number; data: Supplier[] }>
      >(suppliersQueryKeys.list().key);

      await queryClient.cancelQueries({
        queryKey: suppliersQueryKeys.list().key,
      });
      ...
      queryClient.setQueryData(suppliersQueryKeys.list().key, newData);

      await fetchDeleteSupplier({
        id: entityItem.id,
      });
    }
  };
  ...
  const mainButton = (
    <Button
      ...
      variant="contained"
      LinkComponent={Link}
      href={`/admin-panel/suppliers/edit/${entityItem.id}`}
          ...
          <Grid container size="auto" wrap="nowrap" spacing={2}>
            <Grid size="auto">
              <Button
                variant="contained"
                LinkComponent={Link}
                href="/admin-panel/suppliers/create"
                color="success"
            ...
            fixedHeaderContent={() => (
              <>
                <TableRow>
                  ...
                  <TableCell>{t("table.lastOrderDate")}</TableCell>
...
export default withPageRequiredAuth(SuppliersPageContent, {
