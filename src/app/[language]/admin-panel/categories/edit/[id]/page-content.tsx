...
import { useEditCategoryService } from "@/services/api/services/categories";
import { useGetCategoryQuery } from "../../queries/queries";
...
const useValidationSchema = () => {
  const { t } = useTranslation("admin-panel-categories-edit");
...
function FormEdit() {
  ...
  const fetchEditCategory = useEditCategoryService();
  const { t } = useTranslation("admin-panel-categories-edit");
  const validationSchema = useValidationSchema();
  const { data: initialData } = useGetCategoryQuery({ id: entityId });
  ...
  const onSubmit = handleSubmit(async (formData) => {
    const { data, status } = await fetchEditCategory({
      id: entityId,
      data: {
        name: formData.name,
