...
import { useDeleteCategoryService } from "@/services/api/services/categories";

const TableCellLoadingContainer = styled(TableCell)(() => ({
  padding: 0,
}));

function Actions({ entityItem }: { entityItem: Category }) {
  const [open, setOpen] = useState(false);
  const { confirmDialog } = useConfirmDialog();
  const fetchDeleteCategory = useDeleteCategoryService();
  const queryClient = useQueryClient();
  const anchorRef = useRef<HTMLDivElement>(null);
  const { t } = useTranslation("admin-panel-categories");
  ...
  const handleDelete = async () => {
    ...
    if (isConfirmed) {
      ...
      const previousData = queryClient.getQueryData<
        InfiniteData<{ nextPage: number; data: Category[] }>
      >(categoriesQueryKeys.list().key);
      ...
      queryClient.setQueryData(categoriesQueryKeys.list().key, newData);
  ...
  const mainButton = (
    <Button
      ...
      variant="contained"
      LinkComponent={Link}
      href={`/admin-panel/categories/edit/${entityItem.id}`}
          ...
          <Grid container size="auto" wrap="nowrap" spacing={2}>
            <Grid size="auto">
              <Button
                variant="contained"
                LinkComponent={Link}
                href="/admin-panel/categories/create"
                color="success"
...
export default withPageRequiredAuth(CategoriesPageContent, {
