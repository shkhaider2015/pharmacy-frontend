...
function Actions({ entityItem }: { entityItem: Customer }) {
  const [open, setOpen] = useState(false);
  const { confirmDialog } = useConfirmDialog();
  const fetchDeleteCustomer = useDeleteCustomerService();
  ...
  const handleDelete = async () => {
    ...
    if (isConfirmed) {
      ...
      queryClient.setQueryData(customersQueryKeys.list().key, newData);
  ...
  const mainButton = (
    <Button
      ...
      variant="contained"
      LinkComponent={Link}
      href={`/admin-panel/customers/edit/${entityItem.id}`}
            ...
            fixedHeaderContent={() => (
              <>
                <TableRow>
                  ...
                  <TableCell>{t("table.companyName")}</TableCell>
...
export default withPageRequiredAuth(CustomersPageContent, {
